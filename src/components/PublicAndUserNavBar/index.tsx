...
export function PublicAndUserNavbar(props: Props) {
  ...
  return (
    ...
    >
      <NavbarBrand className="xs:basis-60 min-w-fit shrink-0 grow-0 basis-auto">
        <Link
          href={homeHref}
      ...
      {showSearchInput() && (
        <SearchInput
          key={`${pathname}-mobile`}
          className="xs:hidden w-[5.75rem] shrink-0"
          compact
        />
      )}
      <NavbarMenuToggle className="xs:hidden ml-0.5 size-10 min-w-10 rounded-xl" />
