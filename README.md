...
> [!TIP]
> 如果你只想先把项目跑起来，本地开发通常只需要默认 SQLite 配置。AI 供应商在管理员后台配置，不需要环境变量。
...
2. 在项目的 Environment Variables 中配置数据库与认证参数。
...
项目通过 Vercel AI SDK 接入 OpenAI-compatible 服务。管理员登录后台后，可在「大模型供应商」页面维护站点级供应商配置，并选择当前启用的供应商。
